...
public abstract class CompositePeriod {
		...
		final org.drip.param.market.CurveSurfaceQuoteSet csqs)
	{
		...
		org.drip.function.definition.R1ToR1 auCreditVol = csqs.creditCurveVolSurface (creditLabel);

		org.drip.function.definition.R1ToR1 auForwardVol = csqs.forwardCurveVolSurface
			(forwardLabel);

		org.drip.function.definition.R1ToR1 auFundingVol = csqs.fundingCurveVolSurface
			(fundingLabel);

		org.drip.function.definition.R1ToR1 auFXVol = csqs.fxCurveVolSurface (fxLabel);

		org.drip.function.definition.R1ToR1 auCreditForwardCorr = csqs.creditForwardCorrSurface
			(creditLabel, forwardLabel);

		org.drip.function.definition.R1ToR1 auForwardFundingCorr = csqs.forwardFundingCorrSurface
			(forwardLabel, fundingLabel);

		org.drip.function.definition.R1ToR1 auForwardFXCorr = csqs.forwardFXCorrSurface
		...
		final org.drip.product.calib.ProductQuoteSet pqs)
	{
		...
		try {
			...
			dblAccrued = accrualDCF (dblValueDate) * cpqs.basis() * dblNotional * dblFX;
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		...
		if (cpqs.containsBaseRate()) {
			...
			try {
				double dblForwardDF = 1. / (1. + org.drip.analytics.daycount.Convention.YearFraction
					(dblStartDate, dblEndDate, cup.couponDC(), false, null, cup.calendar()) *
						cpqs.baseRate());

				if (!prwc.addPredictorResponseWeight (dblStartDate, dblNotional * dblForwardDF)) return null;

				if (!prwc.addDResponseWeightDManifestMeasure ("PV", dblStartDate, dblNotional *
					dblForwardDF))
					return null;
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			...
			if (!prwc.addPredictorResponseWeight (dblEndDate, -1. * dblNotional)) return null;

			if (!prwc.addDResponseWeightDManifestMeasure ("PV", dblEndDate, -1. * dblNotional)) return null;
		} else {
